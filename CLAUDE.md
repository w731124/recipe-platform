...
**同義詞（`synonyms.json`）只能收「對烹飪來說完全可以互相取代」的講法**，不能因為兩個詞長得像、或字面上有包含關係就合併。已知的反例（不可合併）：蒜苗≠蔥、香油≠麻油、花椒粒≠花椒粉、青花椒≠紅花椒。判斷原則：**只要「品種」或「形態／加工方式」任一軸不同，就不是同義詞**，即使原文只用一個籠統的詞（例如單純寫「花椒」），也不要因此把它塞進某個特定品種/形態的同義詞群組——寧可讓它比對不到、由下面的「家族」機制軟提示，也不要製造誤判成「已有」的假陽性。
...
**另一個真實案例（演算法層級的 bug，不是資料層級）**：新增 `"米": ["米", "白米"]` 之後，發現食譜寫「米酒」會被誤判成已有——不是因為同義詞群組設錯，而是因為 `isInPantry` 原本的雙向 substring 比對，只要字串裡「包含」對方就算命中，而「米酒」「味醂」的別名「米霖」字面上都包含「米」這個字根，導致誤判命中「米」這個完全不相關的同義詞群組。中文很多食材名是「基礎詞 + 酒/醋/粉/油/醬/糖」組成完全不同的品項（米→米酒/米醋、花→花椒/花生，以此類推），單一漢字幾乎都只是字根、不是完整食材名。修法是在 `assets/app.js` 加了 `looseMatch()`：兩個字串要嘛完全相等，要嘛「比較短的那一邊至少 2 個字」才允許寬鬆的子字串比對，一個字的詞只接受完全相等。這是通用的演算法防呆，不是逐一資料修正——之後如果又出現類似的誤判，先檢查是不是又踩到「單一漢字當同義詞/家族代表詞」的情況，理論上已經被 `looseMatch` 擋掉了，但如果發現新的漏網案例要在這裡補充說明。
