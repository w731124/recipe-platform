...
**延伸情況：泛稱食材、未指定品種**（例如「菇菇」）——這跟「完全不特定的隨興食材」不完全一樣（已經知道是哪個大類，只是沒說哪個品種），但處理方式比照辦理：**不列入 `ingredients`，保留在步驟文字裡**。判準是「原文有沒有列出具體候選品種」：
- 原文只寫泛稱、**沒有列出候選品種**（例如單純寫「菇菇」，沒說是蘑菇還是香菇還是什麼菇）→ 適用這條規則，不列入 `ingredients`。
- 原文**明確列出多個具體候選品種**（例如「蘑菇、秀珍菇、鴻喜菇、雪白菇」列成一排讓人任選）→ 這種情況改用 `choice_group`（見下方「`choice_group`：擇一群組」），把每個候選品種各自列進 `ingredients`、標同一個 `choice_group`，因為這時候有明確可辨識的候選項目，值得各自追蹤食材庫比對狀態，不像純泛稱那樣無從歸類。
兩者的分界就是「有沒有列出具體候選品種清單」，不是「像不像隨興」的語感問題，遇到類似情況先確認原文有沒有列出候選品種清單再決定套用哪一條。

**第三種選擇：範疇明確、不列候選，但希望在食材清單裡看得到這個範疇**（例如「各種綜合海鮮」——不像「有啥菜加啥菜」那樣完全不特定，也沒有像松露義大利麵的菇類那樣列出具體候選品種，但使用者希望食材清單能顯示「海鮮」這個範疇、並且有庫存比對意義）。這種情況預設還是照上面兩條規則處理（不列入 `ingredients`）；**只有使用者明確表態想讓它出現在食材清單時**，才改用下面這個做法，不要自己主動幫每個「隨興但有範疇」的食材都加上去：

1. 在 `ingredients` 裡新增一個代表這個範疇的項目（例如 `{ "name": "海鮮（各種綜合，依喜好選擇）", "category": "海鮮" }`）——`category` 要填這個範疇實際對應的 `pantry_categories` 值（範疇名稱剛好等於分類名稱時很直觀，例如「海鮮」；但像「菇類」這種範疇本身不是正式分類，食材庫裡的菇類其實歸在「蔬菜」分類，這時候 `category` 要填「蔬菜」，不能填「菇類」）。
2. 到 `data/ingredient_families.json` 新增（或沿用既有的）「範疇代表詞」家族，key 用這個範疇名稱（例如「海鮮」「菇類」），value 是目前食材庫裡屬於這個範疇的所有品項正式名稱（例如 `"海鮮": ["蝦仁"]`）——完全比照既有「起司」家族的做法，不是新機制。
3. 比對結果會是 `maybe`（琥珀色，提示使用者自行確認），不會是 `have`——因為「家族」機制本來就是「同範疇但不確定是哪個品種」的軟提示，不是精確比對，這點跟明確列候選品種的 `choice_group`（可以是 `have`）不一樣。
4. **維護提醒**：之後食材庫新增這個範疇底下的新品項時（例如海鮮類新增「透抽」），記得同步把它加進 `ingredient_families.json` 對應的家族清單，不然新品項不會被這個範疇代表詞帶到——這跟「起司」家族的既有維護方式一致。

這條路徑純粹是資料層面的操作（沿用既有的家族機制），不需要改動 `assets/app.js` 的比對邏輯。
