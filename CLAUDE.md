...
這份白名單同時存在 `data/taxonomy.json` 的 `always_available_whitelist` 欄位（跟這裡的文字說明是同一份清單，只是給前端「食材庫」頁面唯讀顯示用，見下方前端行為說明）。**以後調整白名單時，這裡的文字跟 `taxonomy.json` 的陣列要一起改，不要只改其中一邊**，避免兩處不同步。
...
「食材庫」分頁另外有一個「🧂 國民常備調味料」摺疊區塊（`renderPantryView` 裡的 `whitelistHtml`），純粹讀取 `taxonomy.always_available_whitelist` 顯示給使用者看目前白名單有哪些項目，**唯讀、不能在網站上新增/刪除**——這份清單的增減仍然只能透過 Claude Code 對話調整 `taxonomy.json` 跟本節文字兩處，不要把它跟 `pantry.json` 那 16 個可編輯分類混為一談，也不要因為使用者要求「新增/刪除白名單項目」就直接開放網站寫入權限，這需要另外討論並修改這節規則才能做。
