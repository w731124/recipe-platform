...
```json
{
  ...
  "servings": "string，例如 2人份，原文沒提到就省略",
...
- `amount` 一律存成字串，因為「適量」「少許」這類非數值必然存在，若存成數字型別會在解析時炸掉。原文沒提供份量時，`amount`/`unit` 整個欄位省略，不強迫填值、也不要求你每次補——上傳量一大，逐筆確認份量會讓這個工具失去「快速整理」的意義。`servings`、`time_minutes` 同理，沒提到就省略。
