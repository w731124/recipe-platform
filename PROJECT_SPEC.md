...
- **例外（v0.2 新增）**：食材庫（`data/pantry.json`）的新增/刪除是唯一允許網站執行期直接寫入的資料。做法是讓瀏覽器直接呼叫 GitHub Contents API，用你自己貼上、存在瀏覽器 localStorage 的 fine-grained token（只開這個 repo 的 Contents 讀寫權限）建立 commit。這個例外只開放給食材庫這一份低風險、你自己常態維護的資料，其他資料（食譜、分類詞彙表、同義詞庫）仍然維持「網站唯讀、寫入靠 Claude Code 離線 + git push」的原則，因為那些資料需要人工 review（份量判斷、同義詞語意）才適合寫入，不適合開放成即時可寫。細節見 `CLAUDE.md`「食材庫網站直接寫入」一節。
